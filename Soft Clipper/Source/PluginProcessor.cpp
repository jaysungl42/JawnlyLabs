...
state (*this, nullptr, juce::Identifier("params"), {
    ...
    std::make_unique<juce::AudioParameterFloat>(
                  "E",
                  "E",
                  juce::NormalisableRange<float>(0, 9, 1),
                  0),
    std::make_unique<juce::AudioParameterFloat>(
                  "mix",
                  "Mix",
    ...
    std::make_unique<juce::AudioParameterFloat>(
                  "algo",
                  "Algorithm",
                  juce::NormalisableRange<float>(1, 6, 1),
                  2),
})
#endif
{
...
void SoftClipperAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    EParam = (float*)state.getRawParameterValue("E");
    mixParam = (float*)state.getRawParameterValue("mix");
    algoParam = (float*)state.getRawParameterValue("algo");
}
...
void SoftClipperAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < buffer.getNumSamples(); i++) {
        for (int channel = 0; channel < totalNumInputChannels; ++channel) {
            ...
            Clipper* clipr = &clippers.getReference(channel);
            channelData[i] = clipr->clipSample(channelData[i], *threshParam, *EParam, *mixParam, *algoParam);
        }
    }
}
...
{
    return new SoftClipperAudioProcessorEditor (*this, state);
}
